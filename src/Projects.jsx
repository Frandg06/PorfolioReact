    ...
    <article className="mt-14">
      ...
      {user.projects.map((item, index) => (
        <article className="grid md:grid-cols-12 gap-6 mb-6 group/project md:h-72" key={`${index}-project`}>
          <a
            className="relative block w-full  md:col-span-5 overflow-hidden cursor-pointer bg-gray-700/50 rounded-xl min-h-64"
            href={item.url}
            target="_blank"
          >
          ...
          <div className="flex flex-col justify-between w-full md:col-span-7 space-y-3 ">
            ...
            <div className="flex gap-4">
              ...
              <a
                href={item.url}
                target="_blank"
                className="flex items-center gap-2 px-5 py-2 font-bold duration-300 bg-white text-black rounded-lg w-fit group/link hover:bg-gray-300"
              >
                Live
